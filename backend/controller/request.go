...
func CreateRequest(c *gin.Context) {
	...
	if tx := entity.DB().Where("subjectid = ?", subject.Subject_ID).First(&subject); tx.RowsAffected == 0 {
...
func ListRequest(c *gin.Context) {
	var request []entity.Request
	if err := entity.DB().Raw("SELECT * FROM request").Scan(&request).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func UpdateRequest(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", request.Request_ID).First(&request); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "user not found"})
		return
	}
