...
func GetSubjectByProfessorandStudenByEnroll(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT e.*,s.* FROM enrolls e JOIN subjects s where s.professor_id = ? AND e.subject_id = ?  AND e.subject_id = s.subject_id GROUP BY e.subject_id = ?", professor_id,subject_id,subject_id).Scan(&subject).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
